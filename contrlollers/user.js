                 ...
                 User.findOne({fullname:req.body.fullname}).exec(function(err,user){
                     if(err){console.log("ERROR");
                         ...
                         if(!user){  
                           ...
                           return res.status(200).json({user:newAdmin})    
                        }
                        else{
            ...
            console.log("Inside add new stackholder =======================");
                ...
                User.findOne({fullname:req.body.fullname}).exec(function(err,user){
                    if(err){console.log({error:"ERROR"});
                        ...
                        return res.status(200).json({user:newstackholder})    
                        }
                        else{
                            return res.status(400).json({error:'Already exists'})
                        }
                          ...
                          if(user){
                              if(!user.resetedPass){
                                    if(!user.isValidPass(req.body.password)){
                                        ...
                                        return res.status(400).json({error:"this is your old password please enter new password"});
                                    }
                            }
                            else{
                                return res.status(400).json({error:"unable to process"});
                            }
